...
if(!isset($_SESSION['userId']) && !empty($_COOKIE['rememberMe'])){
    ...
    if(!$result){
        echo  '<div class="error">There was an error running the query.</div>'; 
        exit;
    }
    $count = mysqli_num_rows($result);
    if($count !== 1){
        echo '<div class="error">Remember me process failed!</div>';
        exit;
    }
    $row = mysqli_fetch_array($result, MYSQLI_ASSOC);
    //if authentificator2 does not match
    if(!hash_equals($row['f2authentificator2'], $f2authentificator2)){
        echo '<div class="error">hash_equals returned false.</div>';
    }else{
        ...
        (`authentificator1`, `f2authentificator2`, `userId`, `expires`) VALUES ('$authentificator1', '$f2authentificator2', '$userId', '$expiration')";
        $result = mysqli_query($link, $sql);
        if(!$result){
            echo  '<div class="error">There was an error storing data to remember you next time.</div>';  
        }
